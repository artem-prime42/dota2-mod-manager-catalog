...
const siteRoot = '/home/artem/dota2skins';
...
const heroesPath = path.join(siteRoot, 'app', 'lib', 'heroes.ts');
...
function loadHeroIcons() {
  ...
  const text = fs.readFileSync(heroesPath, 'utf8');
...
function buildCatalog() {
  ...
  for (const line of lines) {
    ...
    if (currentSection === 'hero' && heroEntryMatch && !currentObject) {
      ...
      heroEntries.push({ slug: currentHeroKey, name: formatHeroLabel(currentHeroKey), items: [] });
      currentObject = {};
      continue;
    }
    ...
    if (!currentObject || !propertyMatch) continue;

    const [, key, valueText] = propertyMatch;
    if (key === 'id') currentObject.id = parseStringValue(valueText);
    else if (key === 'title') currentObject.name = parseStringValue(valueText);
    else if (key === 'author') currentObject.author = parseStringValue(valueText);
    else if (key === 'category') currentObject.category = parseStringValue(valueText);
    else if (key === 'hero') currentObject.hero = parseStringValue(valueText);
    else if (key === 'imageUrl') currentObject.preview = parseStringValue(valueText);
    else if (key === 'invokerSlot') currentObject.slot = parseStringValue(valueText);
    else if (key === 'downloadUrl') currentObject.file = parseStringValue(valueText);
    else if (key === 'createdAt') currentObject.createdAt = parseStringValue(valueText);
  }
  ...
  for (const entry of modEntries) {
    ...
    const heroSlug = entry.hero || null;
...
const outPath = path.join('/home/artem/Other/catalog-repo', 'catalog.json');
